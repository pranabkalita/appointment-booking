<div class="bg-gray-200 max-w-sm mx-auto m-6 p-5 rounded-lg">
    <form wire:submit.prevent="createBooking">
        <div class="mb-6">
            ...
            <select name="for" id="for" class="bg-white h-10 w-full border-none rounded-lg" wire:model="state.service">
        ...
        <div class="mb-6 {{ !$employees->count() ? 'opacity-25': '' }}">
            ...
            <select name="for" id="for" class="bg-white h-10 w-full border-none rounded-lg" wire:model="state.employee" {{ !$employees->count() ? 'disabled="disabled"' : '' }} >
        ...
        @if($this->hasDetailsToBook)
            ...
            <button type="submit" class="bg-indigo-500 text-white h-11 px-4 text-center font-bold rounded-lg w-full">Book now</button>
        @endif
    </form>
</div>
